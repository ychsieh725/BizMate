...
**A6 已完成兩次實測**（2026-08-17 首測、2026-08-18 修復後重測），下表為重測值（完整分析見 [`docs/agent-eval-a6-comparison.md`](../../agent-eval-a6-comparison.md)）：

| 指標 | 單步 baseline | agent loop | 判定 |
| :--- | ---: | ---: | :--- |
| 欄位抽取準確率 | 98.5% | 98.0% | ✅ 持平（McNemar p = 1.00） |
| 幻覺率 | 0% | **0%** | ✅ 維持 0%（0/53，95% CI [0%, 6.8%]） |
| 每案成本 | $0.000474 | $0.001443 | ⚠️ **3.0×** |
| P95 延遲 | 11010ms | 19502ms | ⚠️ **量測不可信**：同一批案例兩次執行差 2 倍，而 token 量只增 15%。單次執行不足以推論延遲，此門檻待改為多次量測取中位數 |
| **客戶平均被問題數** | **1.47** | **1.53** | ❌ baseline 已與標註完全一致，agent 無發揮餘地 |

最後一列是本次改動的**產品價值指標**，也是唯一的產品面理由。修復後 baseline 的
反問數已與標註完全一致（1.47 = 1.47），agent 要贏就得問得比正確答案還少，
那不是能力問題而是定義問題。硬門檻雖然全數通過（故 A7 解除封鎖），
但**不建議於 A8 開啟 flag**，理由與後續建議見 A6 對照報告。

首測時 agent 三個判準中兩個落後，根因是**計價單位從未給模型**：`rate_card_base.unit`
記載「貼圖表情包」按組計價，但兩側都只把 subtype 名稱給模型。修復後（2026-08-18）
兩側皆受益，`illu-003`「一組貼圖，八款」的 8 倍錯價消除，報價偏差由最大 700%
降為 0%，agent 與 baseline 在三個判準上持平。
...
3. ~~「客戶平均被問題數」的 baseline 尚未量測~~ → **A6 已量測（修復後）：baseline 1.47、agent 1.53、標註 1.47**。baseline 與正確答案完全一致，agent 在此任務上沒有發揮餘地，本次改動的產品價值理由未成立
