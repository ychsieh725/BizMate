...
三者都是**防災上限，不是目標值**。正常情形下一次 loop 約 3–5 步、10 秒內、
成本在 $0.0005 量級；預算設得比那寬鬆得多，是為了讓「撞到預算」明確代表
異常，而非日常。真正的效能約束來自 eval 的基準線（見設計文件〈基準線重建〉）。
"""
...
# 一次 loop 最多幾步。設計文件 v3 的估計值，待 A4 實測後調整。
MAX_AGENT_STEPS = 8

# 累積延遲上限。分層預算：TS 端逾時 90s > 本值 60s > 單次 Gemini 呼叫約 1.3s。
MAX_AGENT_LATENCY_MS = 60_000

# 累積成本上限。約為現行每案成本（$0.000442）的 22 倍——刻意寬鬆，
# 它是防災上限而非預期值。
