...
| 8.4 | 部署（Vercel + Supabase 免費層，實測執行上限） | ⏳ 待處理 | 高 | 5.9 | 2h | NFR-3；含 Resend 網域驗證（SPF/DKIM）；CI 接入待辦已獨立為 8.5；作品集前提：需產出可公開訪問的 URL；含 Vercel 超時實測（SAD R-1） |
| 8.5 | verify:* scripts + Eval Runner 接進 GitHub Actions CI | ⏳ 待處理 | 高 | 8.4, 7.2 | 3h | 修復 5.6 code review 指出的靜默回歸窗口（租戶隔離只由手動腳本守門，repository 層刪掉 merchant_id 過濾時單元測試不會紅）；CI 兩道閘門：(1) verify scripts 對真實 dev DB 驗證隔離不變式 (2) Eval Runner 跑 golden set，抽取準確率/忠實度低於基準線即擋合併 |
...
| **Portfolio-Ready** | 線上可訪問 + CI 雙閘門 + 指標驅動證據齊備 | 8.4, 8.5, 7.1, 7.2, 6.4 | ⏳ 待處理 |
