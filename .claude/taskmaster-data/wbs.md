...
| 5.8 | MT-M5：Email 寄送（Resend + 報價信模板） | ⏳ **下一個** | 高 | 5.7 | 1d | 計畫 §6；renderQuoteEmail 純函式（快照測試）；POST /[id]/send → email_sent → sent（寫 sent_at）；reply_to=商家 email；寄失敗停 confirmed 可重寄；verify-email.ts |
...
| MT-M5: 報價寄達 | 確認→客戶信箱收到報價信→quote 進終態 sent | 5.8 | ⏳ **下一個** |
