...
| 5.6 | MT-M4a：報價列表 + 詳情（quotes API + UI） | 🔄 進行中 | 高 | 5.4 | 1d | GET /api/dashboard/quotes?status=、GET /[id]（line items + 抽取欄位 + 澄清歷程 + 原始描述）；保守估算標示 |
...
| MT-M4: 後台終審 | 客戶送單→後台看到→調金額→確認 | 5.6-5.7 | 🔄 進行中（5.6） |
