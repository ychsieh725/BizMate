...
| 5.7 | MT-M4b：調金額 + 確認（quote_confirmed 事件落地） | 🔄 進行中 | 高 | 5.6 | 1d | PATCH /[id]（限 awaiting_review）、POST /[id]/confirm → 狀態機 quote_confirmed → confirmed；quoteReviewService |
...
| **8. 貫穿性任務（每階段並行）** | | | | | | |
| 8.1 | 單元 + 整合測試（TDD，80%+ 覆蓋率） | 🔄 進行中 | 高 | 各實作 | 貫穿 | 現況 320 測試綠；每個 5.x 任務先寫測試；跨租戶隔離是 5.5+ 的必測項（5.5/5.6 已補齊） |
...
| MT-M4: 後台終審 | 客戶送單→後台看到→調金額→確認 | 5.6-5.7 | 🔄 進行中（5.6 ✅，5.7 下一個） |
