...
| 6.8 | Parser 欄位值域約束（subtype 等列舉欄位對齊 rate card） | ⏳ 待處理 | 高 | 7.1 | 3h | **7.1 golden set 首跑抓到的 HIGH 缺陷**：Parser prompt 未告知合法值域，抽出「LOGO」「landing page」「UI設計」「公司形象官網」等原文詞，而 rateCardRepository.findBase 用 `.eq("subtype", …)` 精確相等查表 → 查無 → computeBasePricing 回 outOfScope → final_amount 為 null，「自動報價」退化為人工填價。設計不對稱佐證：作者已在 normalizeLicenseScope 註解寫明「抽取值多變（商用/商業用途）」並做包含式比對，卻未對 subtype 做同等處理。三個候選解法待評估：(a) Parser 傳入該商家 rate card 的 subtype 清單，schema 用 z.enum 限縮（治本，但 Parser 需依賴 pricing 資料）(b) 加 normalizeSubtype 模糊比對（與 license_scope 一致，但 13 個 subtype 模糊比對易誤判）(c) findBase 改模糊查詢（風險最高，錯配即錯價）。次要同源問題：feature_modules 的「明說不需要（無）」被抽為 null（會反問客戶已回答的事）、中文數字未換算（「二十天」→ 20）、coloring_complexity 措辭未收斂（「不用上色」vs「線稿」）。修完應重跑 verify:golden-set 對比 81.4% 基準線 |
