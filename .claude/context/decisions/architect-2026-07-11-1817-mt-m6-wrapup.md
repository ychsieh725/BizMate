...
### 1. Rate limit 雙桶：OR 邏輯，兩桶同規則（code review 覆核後維持）
...
**code review 覆核**：code-quality-specialist 在最終審查中提出 HIGH 等級疑慮——slug 桶是「整個商家、跨所有客戶合計」計數，10 次/小時對生意好的商家可能誤傷真實客戶（第 11 位當小時內的客戶會被擋）。此疑慮技術上成立，但直接牴觸使用者先前已明確拍板的決策，故未逕自變更，而是把 review 的具體理由帶回去給使用者覆核。使用者維持原決定：MVP 階段目標用戶（個人接案者）流量不太可能單小時破 10 人送單，真的碰到再回頭調。
...
## Code Review 結果與處理（併回 main 前的最終覆核）

code-quality-specialist 對整條分支（3d0bc1c..feat/mt-m6-wrapup）的審查：🟡，0 CRITICAL、1 HIGH、2 MEDIUM、3 LOW。

- **HIGH（slug 桶規則）**：見上方「1. Rate limit 雙桶」——已帶回使用者覆核，維持原決定。
- **MEDIUM（SettingsForm SSR/client hydration mismatch）**：`目前連結` 預覽用 `typeof window !== "undefined"` 判斷是否顯示 origin，SSR 與 client 渲染輸出不一致。**已修復**（commit `db3b555`）：移除 origin 拼接，只顯示相對路徑，徹底消除渲染分歧來源，不是繞過。這是唯一一個 mock 單元測試與 repository 層 verify script 都測不到、只有真實渲染會現形的 bug class。
- **MEDIUM（SettingsForm 零元件測試）**：沿用專案既有慣例（`ServicesTable.tsx`/`QuoteActions.tsx` 等 UI 元件皆無測試，測試集中在 route/service/schema 層），非本次任務新增的缺口。維持現狀，記錄為技術債。
- **LOW**：改 slug 靜默失效舊連結（UX 缺口，非程式錯誤）、`display_name` 未 trim（與 `onboardingSchemas.ts` 現況一致，非新回歸）、slug 桶在查 merchant 前就計數（受 IP 桶封頂，非真實 DoS 風險）——三者皆記錄備查，不阻擋合併。

## 行動項目

- [ ] 建議合併後找時間由使用者親自用瀏覽器登入 dev 商家，走一次 `/dashboard/settings` 的改名/改 slug/撞號流程，補上這段目前缺失的真人驗證
- [ ] 若未來觀察到 slug 桶真的擋到合法商家流量，回頭把 slug 桶規則與 IP 桶脫鉤（獨立、更寬鬆的 limit/window）
- [ ] UI 元件測試（SettingsForm 及既有的 ServicesTable/QuoteActions）目前皆無覆蓋，屬專案既有技術債，非本次任務新增；若要收斂建議整批處理而非逐一補
